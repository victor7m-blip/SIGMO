...
export default function PesquisaMaterial({
  ...
  function estaSelecionado(material) {
    return itensSelecionados.some(
      (item) => item.id === material.id && item.tabela_origem === material.tabela_origem
    )
  }
    ...
    <section className="pagar-material-card">
      <div className="pagar-material-card-header">
        ...
        <div className="pagar-material-results-head">
          <strong className="pagar-material-count">{resultados.length} encontrados</strong>
          <button type="button" className="pagar-material-refresh" disabled={loading} onClick={carregarMateriais}>
      ...
      <div className="pagar-material-search">
        ...
        />
        <button type="button" onClick={onAbrirQrCode}>Ler QR Code</button>
      </div>

      {erro && <div className="pagar-material-inline-error">{erro}</div>}
      ...
      <div className="pagar-material-table-wrap">
        <table className="pagar-material-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="pagar-material-table-empty">Carregando materiais...</td></tr>
            ) : resultados.map((material) => {
              const selecionado = estaSelecionado(material)
              return (
                <tr key={[material.tabela_origem, material.id].join('-')}>
                  <td><strong>{material.patrimonio}</strong></td>
                  ...
                  <td>
                    <span className={['pagar-material-badge', material.disponivel ? 'is-success' : 'is-warning'].join(' ')}>
                      {material.status}
                    </span>
                  </td>
                  <td>
                    <button
                      ...
                      disabled={selecionado || !material.disponivel}
            ...
            {!loading && resultados.length === 0 && (
              <tr><td colSpan={6} className="pagar-material-table-empty">Nenhum material encontrado nesta origem.</td></tr>
            )}
          </tbody>
        </table>
      </div>
